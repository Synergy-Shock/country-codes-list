...
- Area Codes: The national area codes that follow the calling code, as `string[]`. This is how countries that share a calling code are told apart: every member of the [North American Numbering Plan](https://en.wikipedia.org/wiki/North_American_Numbering_Plan) has `countryCallingCode: "1"` and is identified by its area code (Jamaica `["876", "658"]`, Barbados `["246"]`, …). Empty for countries that have no such split, and for the US and Canada, whose hundreds of area codes are out of scope for this package (Canada is populated as a legacy exception).
